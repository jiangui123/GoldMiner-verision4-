    ...
    ui(new Ui::MainWindow)
{
   ...
   // ui->lcdNumber_speed->setPalette(pal);
    ...
    pix1.load("://images//iii//hook1.png");//设置钩子，60*50
    pix2.load("://images//iii//hook1.png");
...
void MainWindow::init()
{
    ...
    ui->toolButton1->setStyleSheet("#toolButton{border-image:url(:/images/iii/1.png);}");//设置矿工
    ui->toolButton2->setStyleSheet("#toolButton{border-image:url(:/images/iii/1.png);}");

    ui->lcdNumber->display(time_left);
    ui->lcdNumber_sum->display(miner1->get_sum());
    timer_angle.start();
    timer_clock.start();
    timer_s.start();
}
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    if(miner1->isrotating()==false||miner1->isstopped())
    {
        QPen pen1;
        pen1.setWidth(6);
        painter1.setPen(pen1);
        painter1.setRenderHint(QPainter::Antialiasing, true);
        painter1.drawLine(250,95,250-length1*sin(miner1->get_angle()/rad),95+length1*cos(miner1->get_angle()/rad));
        painter1.drawPixmap(250-length1*sin(miner1->get_angle()/rad),95+length1*cos(miner1->get_angle()/rad),pix1);//绘图的起点
    }
    if(miner2->isrotating()==false||miner2->isstopped())
    {
        QPen pen2;
        pen2.setWidth(6);
        painter2.setPen(pen2);
        painter2.setRenderHint(QPainter::Antialiasing, true);
        painter2.drawLine(450,95,450-length2*sin(miner2->get_angle()/rad),95+length2*cos(miner2->get_angle()/rad));
        painter2.drawPixmap(450-length2*sin(miner2->get_angle()/rad),95+length2*cos(miner2->get_angle()/rad),pix2);//绘图的起点
    }
    else
    {
       painter1.drawPixmap(250+20*sin(miner1->get_angle()/rad),95+20*cos(miner1->get_angle()/rad),pix1);//绘图的起点
       painter2.drawPixmap(450+100*sin(miner2->get_angle()/rad),95+20*cos(miner2->get_angle()/rad),pix2);//绘图的起点
...
void MainWindow::keyPressEvent(QKeyEvent *e)
{
    ...
    if (!miner1->isstopped()&&key ==Qt::Key_S) // 按向下方向键时
    {
...
void MainWindow::mm1()//矿工动作设置
{
    if(!miner1->isrotating())
    {
        if(miner1->isfflag())//矿工的动作设置
            ui->toolButton1->setStyleSheet("#toolButton{border-image:url(:/images/iii/2.png);}");
        else
            ui->toolButton1->setStyleSheet("#toolButton{border-image:url(:/images/iii/3.png);}");
    }
...
void MainWindow::mm2()//矿工动作设置
{
    if(!miner2->isrotating())
    {
        if(miner2->isfflag())//矿工的动作设置
            ...
            ui->toolButton2->setStyleSheet("#toolButton{border-image:url(:/images/iii/3.png);}");
    }
